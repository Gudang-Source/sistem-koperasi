...
class Index extends MX_Controller {
    ...
    function proses_peminjaman(){
        ...
        if($params != null){
            ...
            foreach ($selectData->result_array() as $rowData) {
                $kdanggota = $rowData['kd_anggota'];
            }
    ...
    function showTerbilang($bilangan){
        return rtrim(trim($this->Terbilang($bilangan)));
    }
    ...
    function sumDate($date = "0000-00-00", $tenggang = 0, $tipe = 'date'){
        $return = date("Y-m-d");
        if($date != "0000-00-00" && $tenggang != 0){
            $divDate = explode("-", $date);

            switch ($tipe) {
                case 'date':
                    $return = date("d-m-Y", mktime(0,0,0,$divDate[1], $divDate[2]+$tenggang, $divDate[0]));
                    break;
                case 'month':
                    $return = date("d-m-Y", mktime(0,0,0,$divDate[1]+$tenggang, $divDate[2], $divDate[0]));
                    break;
                case 'year':
                    $return = date("d-m-Y", mktime(0,0,0,$divDate[1], $divDate[2], $divDate[0]+$tenggang));
                    break;
                default:
                    $return = date('d-m-Y');
                    break;
