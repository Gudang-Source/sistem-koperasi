...
<table border="0" width="100%" align="center" class="table-parent">
    <tbody>
        ...
        <tr align="justify">
            <td colspan="2">KOPERASI telah memberikan kepada DEBITUR fasilitas Kredit berupa Kredit "Installment" / Reguler sebesar Rp. <?=number_format($detailPinjaman->row()->jumlah_pinjaman)?> Terbilang <?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->jumlah_pinjaman)?> Jumlah mana tidak termasuk beban bunga dan biaya biaya lainnya.</td>
        </tr>
        <tr align="center">
            <td colspan="2">&nbsp;</td>
        </tr>
        ...
        <tr align="justify">
            <td colspan="2">
                <table>
                    ...
                    <tr>
                        <td valign="top">1.</td>
                        <td>Bunga Terhadap Kredit Installment / Reguler sebesar Rp. <?=number_format($detailPinjaman->row()->pokok_angsuran+$detailPinjaman->row()->bunga_angsuran)?>(<?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->pokok_angsuran+$detailPinjaman->row()->bunga_angsuran)?>) per bulan sejak tanggal penarikan sampai dengan pelunasan.</td>
                    </tr>
                    <tr>
                        <td>&nbsp;</td>
                        <td>
                            <table width="100%" border="0">
                                <tr>
                                    <td width="12" valign="top">a.</td>  
                                    <td colspan="2">Kewajiban harus dibayar setiap bulan:</td>  
                                </tr>
                                <tr>
                                    <td width="12">&nbsp;</td> 
                                    <td width="250">Pokok Kredit:</td>  
                                    <td width="1082" class="border-dot">Rp. <?=number_format($detailPinjaman->row()->pokok_angsuran)?></td>    
                                </tr>
                                <tr>
                                    <td>&nbsp;</td> 
                                    <td>Bunga:</td> 
                                    <td class="border-dot">Rp. <?=number_format($detailPinjaman->row()->bunga_angsuran)?></td>    
                                </tr>
                                <tr>
                                    <td>&nbsp;</td> 
                                    <td>Lainnya:</td>   
                                ...
                                <tr>
                                    ...
                                    <td>Rp. <?=number_format($detailPinjaman->row()->pokok_angsuran+$detailPinjaman->row()->bunga_angsuran)?></td>    
                                </tr>
                                <tr align="center">
                                    <td colspan="2">&nbsp;</td>
                                </tr>        
                                <tr>
                                    <td width="12" valign="top">b.</td>  
                                    <td colspan="2">Untuk sistem angsuran jasa. Pokok pinjaman sebesar Rp <?=number_format($detailPinjaman->row()->jumlah_pinjaman)?>(<?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->jumlah_pinjaman)?>) dibayar lunas pada saat jatuh tempo.</td>    
                                </tr>
                                <tr align="center">
                                    <td colspan="2">&nbsp;</td>
                                </tr>        
                                <tr>
                                    <td width="12" valign="top">c.</td>  
                                    <td colspan="2">Kewajiban pembayaran angsuran dan / atau bunga mana harus dibayar DEBITUR paling lambat tanggal 
                                    <?php
                                        $currentDate = modules::run('admin/index/sumDate', date('Y-m-d'), 2, 'date');
                                        $tenggangWaktu = explode("-", $currentDate);
                                    ?>
                                    <?=$tenggangWaktu[0]?>(<?=modules::run('admin/index/Terbilang',$tenggangWaktu[0])?>) hingga batas waktu pelunasan berakhir.</td>    
                                </tr>
                                <tr align="center">
                                    <td colspan="2">&nbsp;</td>
                                </tr>        
                                <tr>
                                    <td width="12" valign="top">d.</td>  
                    ...
                    <tr>
                        <td valign="top">2.</td>
                        <td>Administrasi 2% Rp. <?=number_format($detailPinjaman->row()->jumlah_pinjaman*(2/100))?>(<?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->jumlah_pinjaman*(2/100))?>)</td>
                    </tr>
                    <tr>
                        <td colspan="2">&nbsp;</td>
                    </tr>
                    <tr>
                        <td valign="top">3.</td>
                        <td>Provisi 1% Rp. <?=number_format($detailPinjaman->row()->jumlah_pinjaman*(1/100))?>(<?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->jumlah_pinjaman*(1/100))?>)</td>
                    </tr>
                    <tr>
                        <td colspan="2">&nbsp;</td>
                    </tr>
                    <tr>
                        <td valign="top">4. </td>
                        <td>Biaya materai secukupnya di bayar di muka Rp. <?=number_format($materai)?>(<?=modules::run('admin/index/Terbilang',$materai)?>)</td>
                    </tr>
        ...
        <tr align="justify">
            <td colspan="2">
                Fasilitas Kredit Installment / Reguler tersebut diberikan untuk jangka waktu <?=$detailPinjaman->row()->jangka_waktu?> (<?=modules::run('admin/index/Terbilang',$detailPinjaman->row()->jangka_waktu)?>) bulan, terhitung sejak tanggal <?=date('d-m-Y')?> sampai dengan <?=modules::run('admin/index/sumDate', date('Y-m-d'), $detailPinjaman->row()->jangka_waktu, 'month')?>
            </td>
        </tr>
        <tr align="center">
            <td colspan="2">&nbsp;</td>
                        ...
                        <td width="99%">Untuk menjamin guna kepastian pembayaran kembali seluruh jumlah kredit dipergunakan termasuk bunga dan biaya-biaya lainnya yang timbul karena pernjanjian ini, DEBITUR menyerahkan barang jaminan kredit berupa : <?=strtoupper($detailJaminan->row()->jaminan)?> dengan
                            ...
                            <?php if($detailJaminan->row()->jaminan == 'bpkb'){ ?>
                                <table width="100%">
                                    <tr>
                                        <td width="1%">-</td>
                                        <td width="15%">Nopol</td>
                                        <td width="1%">:</td>
                                        <td width="83%"><?=$detailJaminan->row()->nopol?></td>                                                                        
                                    </tr>
                                    <tr>
                                        <td>-</td>
                                        <td>Merk</td>
                            ...
                            <?php }else if($detailJaminan->row()->jaminan == 'sertifikat'){ ?>
                                ...
                                <table width="100%">
                                    <tr>
                                        ...
                                        <td width="1%">:</td>
                                        <td width="83%"><?=$detailJaminan->row()->atasnama?></td>                                                                        
                                    </tr>
                                    <tr>
                                        <td>-</td>
                                        <td>Alamat</td>
